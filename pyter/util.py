...
def preprocess_en(s):
    ...
    * 数字に挟まれたピリオドとカンマはくっつける
    ...
    s = re.sub(ur'([^a-zA-Z0-9])(?!\1|$)', ur'\1 ', s) # 前が記号で、後ろが出た文字以外
    s = re.sub(ur'(?<=[a-zA-Z0-9])([^a-zA-Z0-9])', ur' \1', s) # 後ろが記号で、前が記号以外
    s = re.sub(ur'\s+', ur' ', s)  # スペースの個数正規化
    s = re.sub(ur'(\d) ([.,]) (\d)', ur'\1\2\3', s)  # 0 . 1 -> 0.1
    s = re.sub(ur'(Dr|Jr|Prof|Rev|Gen|Mr|Mt|Mrs|Ms) .', ur'\1.', s)  # Mr . -> Mr.
